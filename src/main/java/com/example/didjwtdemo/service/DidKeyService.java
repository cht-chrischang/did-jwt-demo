...
public class DidKeyService {
    ...
    public KeyPair generateP256KeyPair() throws NoSuchAlgorithmException, NoSuchProviderException, java.security.InvalidAlgorithmParameterException {
    ...
    public String getDidFromPublicKey(PublicKey publicKey) {
        ...
        // 加入 multicodec prefix: 0x02, 0x12, 0x01 (P-256)
        byte[] prefixedPub = new byte[pub.length + 3];
        prefixedPub[0] = (byte) 0x02;
        prefixedPub[1] = (byte) 0x12;
        prefixedPub[2] = (byte) 0x01;
    ...
    public String createJwt(String payload, PrivateKey privateKey, PublicKey publicKey) throws Exception {
        // header: {"alg":"ES256","typ":"JWT","did":"..."}
        String did = getDidFromPublicKey(publicKey);
        String headerJson = String.format("{\"alg\":\"ES256\",\"typ\":\"JWT\",\"did\":\"%s\"}", did);
    ...
    public PublicKey getPublicKeyFromDid(String didKey) throws Exception {
        ...
        // 檢查 multicodec prefix: 0x02, 0x12, 0x01
        if (prefixedBytes.length < 3 || 
            prefixedBytes[0] != (byte) 0x02 || 
            prefixedBytes[1] != (byte) 0x12 || 
            prefixedBytes[2] != (byte) 0x01) {
            throw new IllegalArgumentException("Invalid multicodec prefix for P-256");
        }
